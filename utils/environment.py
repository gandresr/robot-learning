...
def create_env_from_metadata(env_meta, env_name = None, render = False, render_offscreen = False, use_image_obs = False):
    """
    Create environment.
    Args:
...
def create_env(env_type, env_name, render = False, render_offscreen = False, use_image_obs = False, **kwargs):
    """
    Create environment.
    Args:
        env_type (int): the type of environment, which determines the env class that will
    ...
    # note: pass @postprocess_visual_obs True, to make sure
    #   images are processed for network inputs
